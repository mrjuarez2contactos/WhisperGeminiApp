...
with st.sidebar:
    ...
    GOOGLE_API_KEY = st.text_input("GOOGLE_API_KEY", type="password", value=os.getenv("GOOGLE_API_KEY",""))
    GEMINI_MODEL = st.text_input("Modelo Gemini", value=os.getenv("GEMINI_MODEL","gemini-2.5-pro"))
    ...
    USER_CONTEXT = st.text_area(
        ...
        value=(
            ...
            "Se clasifica en tallas estándar (16/20, 21/25, ..., 91/110). Estas tallas indican el número de camarones por libra (454g). Para estimar el peso original con cabeza, se calcula: (peso sin cabeza) / 0.70. Ejemplo: un camarón de 9.98g sin cabeza pesaba 9.98 / 0.70 = 14.26g con cabeza.\n\n"
            "**PULPO:**\n"
            "Se maneja por tallas 1/2 y 2/4 (número de pulpos por libra). Por ejemplo, en la talla 1/2, cada pulpo pesa entre 454g y 908g.\n\n"
            "**FILETE DE TILAPIA:**\n"
            "Se maneja por tallas 3/5 y 5/7 (onzas por filete). Se especifica con un porcentaje de agua: 'al 100%' es sin agua añadida, 'al 80%' significa que el 20% del peso es agua."
        ),
        height=400
    )
...
with tab2:
    ...
    # Inicialización del estado de la sesión para que Streamlit recuerde el progreso
    if 'summary_idx' not in st.session_state:
        st.session_state.summary_idx = 0
    if 'summaries' not in st.session_state:
        st.session_state.summaries = {}
    if 'error_paused' not in st.session_state:
        st.session_state.error_paused = False
    ...
    # Botón para reiniciar el proceso si algo sale mal
    ...
    # Si estamos en modo de pausa por un error de cuota
    if st.session_state.error_paused and idx < len(to_summarize):
    ...
    # Si no estamos en pausa y aún quedan archivos por procesar
        ...
        # Botón para procesar el siguiente archivo
        if st.button("▶️ Resumir Siguiente Archivo"):
            with st.spinner(f"Resumiendo {next_file.name}..."):
                try:
                    ...
                    st.session_state.summary_idx += 1  # Avanzar al siguiente índice SOLO si hay éxito
                except Exception as e:
                    if "ResourceExhausted" in str(e) or "429" in str(e):
                        st.session_state.error_paused = True  # Activar el modo de pausa
                    else:
                        st.error(f"Error inesperado con {next_file.name}: {e}")
                        st.session_state.summaries[next_file.name] = f"ERROR: {e}"
                        st.session_state.summary_idx += 1 # Avanzar si es otro tipo de error para no atascarse
            # Forzar re-ejecución para mostrar el siguiente archivo o el estado de pausa
            st.experimental_rerun()
    ...
    # Mostrar siempre los resúmenes que se han generado
...
with tab3:
    ...
    zip_choice = st.radio(
        "¿Qué quieres comprimir?",
        ["Solo RESÚMENES (_geminis)", "Solo TRANSCRIPCIONES (.txt)", "Todo (transcripciones + resúmenes)"],
        index=0
    )
    ...
    with col_a:
        if st.button("Borrar SOLO resúmenes (_geminis)"):
            gem_dir = OUTPUT_DIR / "_geminis"
            if gem_dir.exists():
                count = 0
                for p in gem_dir.glob("*.gem25.txt"):
                    try: p.unlink(missing_ok=True); count += 1
                    except Exception: pass
                st.success(f"Resúmenes eliminados: {count}")
            else:
                st.info("No existe la carpeta _geminis todavía.")
    with col_b:
        if st.button("Borrar SOLO transcripciones (.txt)"):
            count = 0
            for p in OUTPUT_DIR.glob("*.txt"):
                if not p.name.endswith(".gem25.txt"):
                    try: p.unlink(missing_ok=True); count += 1
                    except Exception: pass
            st.success(f"Transcripciones eliminadas: {count}")
    with col_c:
        if st.button("🗑️ Borrar transcripciones YA RESUMIDAS"):
            gem_dir = OUTPUT_DIR / "_geminis"
            if not gem_dir.exists(): st.info("No hay resúmenes, no se puede limpiar nada.")
            else:
                summary_stems_to_delete = {p.stem.replace('.gem25', '') for p in gem_dir.glob("*.gem25.txt")}
                count = 0
                for txt_file in OUTPUT_DIR.glob("*.txt"):
                    if txt_file.stem in summary_stems_to_delete:
                        try: txt_file.unlink(missing_ok=True); count += 1
                        except Exception: pass
                st.success(f"Se eliminaron {count} transcripciones que ya tenían resumen.")
